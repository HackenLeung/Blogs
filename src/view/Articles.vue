    ...
    <div class="articles">
        ...
        <div class="site-content animate">
            ...
            <div id="article-menus">
                <menu-tree :menus="menus.value" child-label="child"></menu-tree>
            </div>
...
function getComment1(count) {
    let List = [];
    mockComment(count, null, null).map(item => {
        let count = 1 + ~~(Math.random() * 5)
        List.push({ comment: item, reply: mockComment(count, item.id, item.fromUserName) })
    })
    return List;
}
function mockComment(count, id, toUserName) {
    let List = [];
    for (let i = 0; i < count; i++) {
        List.push({
            ...
            fromUserId: '',
            fromUserName:  '@cname',
            fromUserAvatar: banners[i],
            content: '@ctitle(20,50)',
            createTime: '123456',
            toUserId: id,
            toUserName: toUserName || '@cname',
...
function getComment() {
        let count = 1 + ~~(Math.random() * 5)
        comments.value = getComment1(count)
}
